...
interface HeaderProps {
  chatWithText: string;
  buddyNameText: string;
}

const Header: React.FC<HeaderProps> = ({ chatWithText, buddyNameText }) => {
  ...
  return (
    <header className="w-3/4 fixed top-0 right-0 z-50 text-sm text-black bg-white py-2 px-4 flex justify-between items-center pt-6 pr-8">
      <div className="flex items-center space-x-4">
        <strong className="text-lg">
          {chatWithText} <span style={{ color: "#8659D3" }}>{buddyNameText}</span>
        </strong>
        {useSystemMessageForAllChats && (
          <span className="flex text-xl relative cursor-pointer">
            <IonIcon icon={informationCircleOutline} />
            <span className="absolute z-10 left-0 ml-4 w-[280px] top-full mt-2 text-sm bg-gray-900 text-white p-2 rounded-md hidden group-hover:block">
              <strong className="underline text-teal-600">System message:</strong>
      ...
      <div className="flex items-center">
        ...
        <button className="p-1">
          <img src={profilePicIcon} alt="Profile icon" className="w-10 h-10" />
        </button>
